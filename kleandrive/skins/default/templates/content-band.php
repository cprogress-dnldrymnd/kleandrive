...
if ( is_array( $planty_template_args ) ) {
	...
	if ( ! empty( $planty_template_args['slider'] ) ) {
	    ...
	    $planty_columns_class = planty_get_column_class( 1, $planty_columns, ! empty( $planty_template_args['columns_tablet']) ? $planty_template_args['columns_tablet'] : '', ! empty($planty_template_args['columns_mobile']) ? $planty_template_args['columns_mobile'] : '' );
				?><div class="<?php echo esc_attr( $planty_columns_class ); ?>"><?php
	}
}
?>
<article id="post-<?php the_ID(); ?>" data-post-id="<?php the_ID(); ?>"
	<?php
	post_class( 'post_item post_item_container post_layout_band post_format_' . esc_attr( $planty_post_format ) );
	...
	planty_show_post_featured( apply_filters( 'planty_filter_args_featured',
		array(
			...
			'thumb_size' => ! empty( $planty_template_args['thumb_size'] )
								...
								in_array( $planty_post_format, array( 'gallery', 'audio', 'video' ) )
	...
	?><div class="post_content_wrap"><?php
		...
		if ( $planty_show_title ) {
			...
			<div class="post_header entry-header">
				<?php
				// Categories
				if ( apply_filters( 'planty_filter_show_blog_categories', $planty_show_meta && in_array( 'categories', $planty_components ), array( 'categories' ), 'band' ) ) {
					do_action( 'planty_action_before_post_category' );
					?>
					<div class="post_category">
						<?php
						planty_show_post_meta( apply_filters(
															...
															'hover_' . $planty_hover, 1
															)
											);
						?>
					</div>
					<?php
					$planty_components = planty_array_delete_by_value( $planty_components, 'categories' );
					do_action( 'planty_action_after_post_category' );
				}
				// Post title
				if ( apply_filters( 'planty_filter_show_blog_title', true, 'band' ) ) {
		...
		// Post content
		if ( ! isset( $planty_template_args['excerpt_length'] ) && ! in_array( $planty_post_format, array( 'gallery', 'audio', 'video' ) ) ) {
			$planty_template_args['excerpt_length'] = 13;
		}
		if ( apply_filters( 'planty_filter_show_blog_excerpt', empty( $planty_template_args['hide_excerpt'] ) && planty_get_theme_option( 'excerpt_length' ) > 0, 'band' ) ) {
			?>
		...
		// Post meta
		if ( apply_filters( 'planty_filter_show_blog_meta', $planty_show_meta, $planty_components, 'band' ) ) {
		...
		// More button
		if ( apply_filters( 'planty_filter_show_blog_readmore', ! $planty_show_title || ! empty( $planty_template_args['more_button'] ), 'band' ) ) {
			if ( empty( $planty_template_args['no_links'] ) ) {
