...
if ( ! function_exists( 'planty_tribe_events_frontend_scripts' ) ) {
	...
	function planty_tribe_events_frontend_scripts( $force = false ) {
		static $loaded = false;
		if ( ! $loaded && (
			current_action() == 'wp_enqueue_scripts' && planty_need_frontend_scripts( 'tribe_events' )
			||
			current_action() != 'wp_enqueue_scripts' && $force === true
			)
		) {
			$loaded = true;
			$planty_url = planty_get_file_url( 'plugins/the-events-calendar/the-events-calendar.css' );
			if ( '' != $planty_url ) {
				wp_enqueue_style( 'planty-the-events-calendar', $planty_url, array(), null );
			}
		}
	}
}
...
if ( ! function_exists( 'planty_tribe_events_frontend_scripts_responsive' ) ) {
	...
	function planty_tribe_events_frontend_scripts_responsive( $force = false ) {
		...
		if ( ! $loaded && (
			...
			$loaded = true;
			$planty_url = planty_get_file_url( 'plugins/the-events-calendar/the-events-calendar-responsive.css' );
			if ( '' != $planty_url ) {
				wp_enqueue_style( 'planty-the-events-calendar-responsive', $planty_url, array(), null, planty_media_for_load_css_responsive( 'tribe-events' ) );
			}
		}
	}
}
