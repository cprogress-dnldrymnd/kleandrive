...
if ( ! function_exists( 'planty_instagram_feed_frontend_scripts_responsive' ) ) {
	...
	function planty_instagram_feed_frontend_scripts_responsive( $force = false ) {
		static $loaded = false;
		if ( ! $loaded && (
			current_action() == 'wp_enqueue_scripts' && planty_need_frontend_scripts( 'instagram_feed' )
			||
			current_action() != 'wp_enqueue_scripts' && $force === true
			)
		) {
			$loaded = true;
			$planty_url = planty_get_file_url( 'plugins/instagram-feed/instagram-feed-responsive.css' );
			if ( '' != $planty_url ) {
				wp_enqueue_style( 'planty-instagram-feed-responsive', $planty_url, array(), null, planty_media_for_load_css_responsive( 'instagram-feed' ) );
			}
		}
	}
}
