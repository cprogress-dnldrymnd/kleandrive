...
if ( ! function_exists( 'planty_cf7_frontend_scripts' ) ) {
	...
	function planty_cf7_frontend_scripts( $force = false ) {
		static $loaded = false;
		if ( ! $loaded && (
			current_action() == 'wp_enqueue_scripts' && planty_need_frontend_scripts( 'cf7' )
			||
			current_action() != 'wp_enqueue_scripts' && $force === true
			)
		) {
			$loaded = true;
			$planty_url = planty_get_file_url( 'plugins/contact-form-7/contact-form-7.css' );
			if ( '' != $planty_url ) {
				wp_enqueue_style( 'planty-contact-form-7', $planty_url, array(), null );
			}
			$planty_url = planty_get_file_url( 'plugins/contact-form-7/contact-form-7.js' );
			if ( '' != $planty_url ) {
				wp_enqueue_script( 'planty-contact-form-7', $planty_url, array( 'jquery' ), null, true );
			}
		}
	}
}
