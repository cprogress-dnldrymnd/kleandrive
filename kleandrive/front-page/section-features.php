<div class="front_page_section front_page_section_features<?php
	$planty_scheme = planty_get_theme_option( 'front_page_features_scheme' );
	if ( ! empty( $planty_scheme ) && ! planty_is_inherit( $planty_scheme ) ) {
		echo ' scheme_' . esc_attr( $planty_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( planty_get_theme_option( 'front_page_features_paddings' ) );
	if ( planty_get_theme_option( 'front_page_features_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$planty_css      = '';
		$planty_bg_image = planty_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$planty_anchor_icon = planty_get_theme_option( 'front_page_features_anchor_icon' );
	$planty_anchor_text = planty_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( planty_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$planty_css      = '';
			$planty_bg_mask  = planty_get_theme_option( 'front_page_features_bg_mask' );
			$planty_bg_color_type = planty_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $planty_bg_color_type ) {
				$planty_bg_color = planty_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$planty_caption = planty_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $planty_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $planty_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $planty_caption, 'planty_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$planty_description = planty_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $planty_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $planty_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $planty_description ), 'planty_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_features_output">
				...
				if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
					...
					if ( ! planty_exists_trx_addons() ) {
						...
						planty_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
					}
				}
				?>
			</div>
