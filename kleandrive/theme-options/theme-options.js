...
jQuery( document ).ready( function() {
	...
	function planty_options_init_fields(e, container) {
				...
				jQuery( this )
					.sortable( {
						...
						update: function( event, ui ) {
							...
							// Mark group as changed
							ui.item.parents('.planty_options_group').find('[data-param]').data( 'param-changed', 1 );
						}
					});
			}
		});
		...
		function planty_options_clone( obj ) {
			...
			clone.find('.planty_options_item_field :input').each( function() {
				...
				if (input.attr('name') && input.attr('name').indexOf("planty_options_field_") === 0) {
					input.on( 'change', function () {
						jQuery( this ).parents('.planty_options_group').find('[data-param]').data( 'param-changed', 1 );
					} );
				}
			});
			//Remove UI sliders
