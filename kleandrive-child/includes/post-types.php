...
class newTaxonomy
{
	function __construct(array $param)
	{
		...
		if (is_array($param['post_type'])) {
			foreach ($param['post_type'] as $post_type_val) {
				add_filter('manage_' . $post_type_val . '_posts_columns', array($this, 'change_table_column_titles'));
				add_filter('manage_' . $post_type_val . '_posts_custom_column', array($this, 'change_column_rows'), 10, 2);
				add_filter('manage_edit-' . $post_type_val . '_sortable_columns', array($this, 'change_sortable_columns'));
			}
		} else {
...
// Add the custom columns to the cpt_team post type:
...
// Add the data to the custom columns for the cpt_team post type:
add_action('manage_cpt_team_posts_custom_column', 'custom_cpt_team_column', 10, 2);
function custom_cpt_team_column($column, $post_id)
{
	switch ($column) {

		case 'team_id':
			echo '<input type="text" value="' . $post_id . '" readonly>';
			break;
	}
}
